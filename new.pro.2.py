...
if 'pending_udhaari' not in st.session_state:
...
def generate_simple_pro_pdf(name, item, inv_date, base_amt, tax_amt, total_amt, gst_status):
    ...
    meta_data = [
        ...
        [Paragraph(f"<b>Invoice Type:</b> {gst_status}", normal_style), Paragraph("", right_style)]
    ]
...
st.markdown(f"""
    <div class="mobile-card">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <b style="font-size: 15px;">📊 Aaj Ki Sales</b>
...
st.markdown(f"""
    <div class="mobile-card">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <b style="font-size: 15px;">⏳ Udhaari / Pending</b>
...
def tax_selection_popup(name, item, inv_date, amt, p_type):
    ...
    if with_gst:
        ...
        st.session_state.pdf_buffer = generate_simple_pro_pdf(name, item, inv_date, amt, tax, final_amount, "Tax Invoice (5% GST)")
        st.session_state.last_party = name
        process_and_save(name, final_amount, p_type)
        
    if without_gst:
        st.session_state.pdf_buffer = generate_simple_pro_pdf(name, item, inv_date, amt, 0, amt, "Bill of Supply (0% GST)")
        st.session_state.last_party = name
